---
title: "Congruence Coefficient Analysis"
author: "Vinita Vader"
output:
  html_document: 
    toc: yes
...
final_data <- function(dataType){
  ...
  list_sheets <- map(sheets, ~readxl::read_xlsx(file$value, sheet = .x, n_max = 373))
  list_sheets
}
...
```{r}
#splits each list into two dataframes called females and males
...
l <- list(females, males)
names(l) <- c("females", "males")
l #output the list
}
...
congrunce_mat_unnamed <- function(dataType){
  ...
  #ips.raw_data <- final_data("Ipsatized")
  
  #split the lists into females and males dfs
  list_all <- map(ips.raw_data, split_list)
  
  #get congruence coefficients for each list
 map(list_all, ~psych::factor.congruence(.x$females, .x$males))
}
...
retain_rows <- mat_long %>%
  ...
  mutate(id_f = str_extract(Var1 , "[^_]*$"), 
         id_m = str_extract(Var2 , "[^_]*$"), 
         id = paste0(id_f, "_", id_m)) %>% 
  select(-id_f, - id_m)
retain_rows
 }
 
#Function to compute congruence coefficients
...
```{r}
#In Hungarian $pairs 
# [,1 ] -rownames - "females"; [,2] - colnames - "males"
...
filename <- "/Users/vinitavader/Desktop/Type-noun/data/models_cc.xlsx"

rio::export(list(Orig_models = all_avg.cc_r, Ips_models = all_avg.cc_z), filename)
```
